...
export class ProviderManager {
  ...
  async initializeProviders(forceInitialize: boolean = false) {
    ...
    console.log(`当前环境: forceInitialize=${forceInitialize}, enableSync=${this.settings.enableSync}`);
    console.log(`当前enabledProviders=${JSON.stringify(this.settings.enabledProviders || [])}`);
    console.log(`当前WebDAV配置: ${this.isWebDAVConfigComplete(this.settings.providerSettings?.webdav) ? '完整' : '不完整'}, enabled=${this.settings.providerSettings?.webdav?.enabled}`);
    ...
    if (forceInitialize) {
      ...
      if (webdavConfigured) {
        console.log('强制初始化模式：检测到WebDAV配置完整，允许初始化');
    ...
    if (!this.settings.enabledProviders || this.settings.enabledProviders.length === 0) {
      console.log('没有启用的存储提供商，请在设置中至少启用一个提供商');
      console.log('当前同步开关状态:', this.settings.enableSync);
      console.log('当前WebDAV状态:', this.settings.providerSettings?.webdav?.enabled ? '已启用' : '未启用');
      console.log('当前WebDAV配置状态:', 
      ...
      if (forceInitialize && this.isWebDAVConfigComplete(this.settings.providerSettings?.webdav)) {
        console.log('强制初始化模式：WebDAV配置完整但不在启用列表中，临时添加');
        ...
        // 记录更新后的状态
        console.log(`临时添加后enabledProviders=${JSON.stringify(this.settings.enabledProviders)}`);
      } else {
        return;
      }
    }
    ...
    // 记录当前提供商映射状态
    console.log(`初始化前，当前提供商映射大小: ${this.storageProviders.size}`);
    if (this.storageProviders.size > 0) {
      console.log('当前已有的提供商:');
    ...
    if (this.storageProviders.has('webdav') && 
      ...
      try {
        ...
        if (provider) {
          console.log('保存现有WebDAV提供商实例，以备初始化失败时恢复');
          existingWebDAVProvider = provider;
        }
      } catch (e) {
        console.log('获取现有WebDAV提供商时出错，将创建新实例', e);
      }
    }
    
    // 先清空现有提供商
    try {
      console.log('清空现有提供商映射');
      this.storageProviders.clear();
      console.log('提供商映射已清空，当前大小: 0');
    } catch (error) {
      console.error('清空提供商映射时出错:', error);
      // 创建新的映射对象，以防止错误
    ...
    console.log(`准备初始化 ${enabledProviders.length} 个提供商: ${enabledProviders.join(', ')}`);
    
    for (const providerType of enabledProviders) {
      totalProvidersAttempted++;
      try {
        console.log(`开始初始化提供商: ${providerType}`);
        const result = await this.initializeProvider(providerType);
        if (result) {
          successfulProviders++;
          console.log(`提供商 ${providerType} 初始化成功`);
          
          // 验证提供商是否真的被添加到映射
          if (!this.storageProviders.has(providerType)) {
            console.error(`警告: 提供商 ${providerType} 报告初始化成功，但未在映射中找到`);
            ...
            if (providerType === 'webdav' && 
              ...
              try {
                console.log('尝试手动创建WebDAV提供商实例');
                const webdavProvider = new WebDAVProvider(
                  this.settings.providerSettings.webdav, 
                  this.app
                );
                this.storageProviders.set('webdav', webdavProvider);
                console.log('手动创建WebDAV提供商成功');
              } catch (e) {
                console.error('手动创建WebDAV提供商失败:', e);
              }
            }
          }
        } else {
          console.error(`提供商 ${providerType} 初始化失败`);
        }
      } catch (error) {
        console.error(`初始化提供商 ${providerType} 时出错:`, error);
        
        // 如果是WebDAV提供商初始化失败，且之前有保存有效实例，尝试恢复
        if (providerType === 'webdav' && existingWebDAVProvider) {
          console.log('WebDAV初始化失败，尝试恢复之前的有效实例');
          this.storageProviders.set('webdav', existingWebDAVProvider);
          successfulProviders++;
          
          // 验证恢复是否成功
          if (this.storageProviders.has('webdav')) {
            console.log('WebDAV提供商恢复成功');
          } else {
            console.error('WebDAV提供商恢复失败，映射中未找到');
          }
        }
      }
    }
    ...
    if (this.settings.enabledProviders.includes('webdav') && 
      ...
      console.log('WebDAV在启用列表中但未成功初始化，再尝试一次');
      ...
      try {
        ...
        this.storageProviders.set('webdav', webdavProvider);
        successfulProviders++;
        console.log('WebDAV提供商最终初始化成功');
      } catch (error) {
        console.error('WebDAV提供商最终初始化失败:', error);
      }
    }
    
    // 输出初始化后的提供商信息
    console.log(`已尝试初始化 ${totalProvidersAttempted} 个存储提供商，成功: ${successfulProviders}`);
    if (this.storageProviders.size > 0) {
      console.log(`已成功初始化的存储提供商 (${this.storageProviders.size}):`);
      for (const [key, provider] of this.storageProviders.entries()) {
        console.log(`- ${key}: ${provider.getName()}`);
      }
    } else {
      console.log('未能成功初始化任何提供商，请检查提供商配置和启用状态');
    }
  }
  ...
  validateAndFixSettings(): boolean {
    ...
    // 确保基本设置结构存在
    if (!this.settings) {
      console.error('设置对象不存在');
      return false;
    }
    
    // 确保providerSettings存在
    if (!this.settings.providerSettings) {
      console.log('初始化providerSettings为空对象');
    ...
    if (!this.settings.enabledProviders) {
      console.log('初始化enabledProviders为空数组');
      this.settings.enabledProviders = [];
      needSave = true;
    }
    ...
    if (webdavConfigured) {
      ...
      console.log(`当前WebDAV启用状态: ${isWebDAVEnabled ? '已启用' : '未启用'}`);
      console.log(`当前WebDAV在提供商列表中: ${isInEnabled ? '是' : '否'}`);
      
      // 验证UI中开关状态与启用状态的一致性
      // 改进: 更主动地修复状态不一致问题，确保WebDAV在启用列表中且被标记为启用
      if (!isWebDAVEnabled || !isInEnabled) {
        if (!isWebDAVEnabled && webdavSettings) {
          console.log('修复: 将WebDAV标记为已启用');
          webdavSettings.enabled = true;
          needSave = true;
        }
        
        if (!isInEnabled) {
          console.log('修复: 将WebDAV添加到启用列表');
          this.settings.enabledProviders.push('webdav');
          needSave = true;
        }
      }
      ...
      if (isWebDAVEnabled || (webdavSettings && webdavSettings.enabled)) {
        ...
        if (this.settings.syncInterval === 0) {
          console.log('修复：WebDAV已启用，但同步间隔为0，设置为默认值5');
          this.settings.syncInterval = 5;
          needSave = true;
        }
      }
    } else {
      console.log('WebDAV配置不完整');
      
      // 如果WebDAV配置不完整，确保它不在启用列表中
      if (this.settings.enabledProviders.includes('webdav')) {
        console.log('修复: WebDAV配置不完整但在启用列表中，移除之');
      ...
      if (this.settings.enabledProviders.length === 0 && this.settings.enableSync) {
        console.log('修复: 没有启用的提供商但全局同步已启用，禁用全局同步');
      ...
      for (const providerType of this.settings.enabledProviders) {
        if (providerType === 'webdav' && this.settings.providerSettings?.webdav) {
          if (this.settings.providerSettings.webdav.enabled !== true) {
            console.log('修复: WebDAV在启用列表中但未标记为启用，设置其为启用');
            this.settings.providerSettings.webdav.enabled = true;
            needSave = true;
          }
        }
    ...
    // 如果进行了任何修复，输出修复后的状态
    if (needSave) {
      console.log('修复后设置状态 - 同步启用:', this.settings.enableSync);
      console.log('修复后设置状态 - 启用的提供商:', this.settings.enabledProviders);
      console.log('修复后设置状态 - WebDAV启用:', 
        this.settings.providerSettings && 
        this.settings.providerSettings.webdav ? 
        this.settings.providerSettings.webdav.enabled : false);
    } else {
      console.log('设置一致性验证完成，无需修复');
    }
  ...
  private isWebDAVConfigComplete(settings: any | undefined | null): boolean {
    // 首先检查整个设置对象是否存在
    if (!settings) {
      console.log('WebDAV配置不完整：整个设置对象不存在');
      return false;
    }
    ...
    if (isComplete) {
      console.log('WebDAV配置完整性检查通过');
    } else {
      console.log('WebDAV配置不完整，缺少必要信息');
    }
  ...
  public async initializeProvider(providerName: StorageProviderType): Promise<boolean> {
    ...
    try {
      ...
      if (providerName === 'webdav') {
        ...
        console.log(`未知的提供商类型: ${providerName}`);
        return false;
      }
      ...
      if (storageProvider) {
        ...
        console.log(`提供商 ${providerName} 添加到映射结果: ${wasAdded ? '成功' : '失败'}`);
        
        if (!wasAdded) {
          console.error(`提供商 ${providerName} 创建后未成功添加到映射，这不应该发生`);
          ...
          if (!retrySuccess) {
            console.error(`再次尝试将 ${providerName} 添加到映射失败`);
            return false;
          } else {
            console.log(`再次尝试添加 ${providerName} 到映射成功`);
          }
        }
        
        console.log(`提供商 ${providerName} 初始化成功，当前映射大小: ${this.storageProviders.size}`);
        return true;
      }

      console.log(`无法为提供商 ${providerName} 创建实例`);
      return false;
    } catch (error) {
      console.error(`初始化提供商 ${providerName} 时发生错误:`, error);
      return false;
    }
  }
  ...
  public async ensureProvidersInitialized(forceInit: boolean = false): Promise<boolean> {
    console.log(`确保提供商初始化 - forceInitialize: ${forceInit}, 当前映射大小: ${this.storageProviders.size}`);
    // 保存原始同步状态
    ...
    try {
      // 强制初始化时临时启用同步
      if (forceInit) {
        console.log("强制初始化标志设置为 true，临时启用同步");
        this.settings.enableSync = true;
      ...
      if (webdavEnabled && webdavSettingsComplete) {
        ...
        console.log(`WebDAV启用和配置完整, 在映射中存在: ${hasWebDAV}, 映射大小: ${this.storageProviders.size}`);
        
        if (!hasWebDAV) {
          console.log("WebDAV配置完整但未在映射中找到, 尝试紧急初始化");
          // 紧急尝试单独初始化WebDAV
          try {
            const webdavSettings = this.settings.providerSettings?.webdav;
            ...
            if (webdavSettings && this.isWebDAVConfigComplete(webdavSettings)) {
              ...
              console.log(`WebDAV紧急初始化结果: ${retrySuccess ? '成功' : '失败'}`);
              
              if (!retrySuccess) {
                console.log('尝试第二种紧急初始化方法');
                await this.initializeProvider('webdav');
              }
            } else {
              console.error('WebDAV设置不完整，无法执行紧急初始化');
              await this.initializeProvider('webdav');
            }
          } catch (err) {
            console.error("WebDAV紧急初始化失败:", err);
            // 再次尝试通过常规方法初始化
            await this.initializeProvider('webdav');
          }
        }
      } else {
        console.log(`WebDAV状态 - 配置完整: ${webdavSettingsComplete}, 已启用: ${webdavEnabled}`);
      }

      // 如果没有提供商被初始化，返回错误
      if (this.storageProviders.size === 0) {
        console.log("常规初始化未能添加提供商，将在同步时通过恢复机制处理");
        return false;
      }

      console.log(`提供商初始化完成, 映射大小: ${this.storageProviders.size}`);
      console.log(`提供商包括: ${Array.from(this.storageProviders.keys()).join(', ')}`);
      return true;
    } catch (error) {
      console.error("初始化提供商时出错:", error);
      return false;
    } finally {
      // 恢复原始同步状态
      if (forceInit && this.settings.enableSync !== originalSyncEnabled) {
        console.log(`恢复原始同步状态: ${originalSyncEnabled}`);
        this.settings.enableSync = originalSyncEnabled;
        // 保存设置以确保状态被正确保存
        try {
          await this.settingsSaver.saveSettings();
          console.log("原始同步状态已恢复并保存");
        } catch (err) {
          console.error("保存恢复的同步状态时发生错误:", err);
        }
      }
    }
  }
  ...
  private getProviderMappingSize(): number {
    try {
      ...
      console.error("获取提供商映射大小时出错:", err);
      return -1;
    }
  }
} 